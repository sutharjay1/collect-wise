...
const menuItems = [
  ...
  },
  {
    ...
    label: "Domain",
    subItems: [
      { icon: <Globe className="h-5 w-5" />, mode: DomainMode.DASHBOARD },
      {
        icon: <ShieldCheck className="h-5 w-5" />,
        mode: DomainMode.DEBTORS,
      },
      {
        icon: <Network className="h-5 w-5" />,
        mode: DomainMode.PAYMENTS,
      },
      { icon: <Wifi className="h-5 w-5" />, mode: DomainMode.SETTINGS },
    ],
  },
];
                ...
                <li key={index} className="relative">
                  {item.href ? (
                    <Button
                      variant="ghost"
                      className={cn(
                        "text-text/90 hover:text-text flex h-9 items-center justify-between gap-2 rounded-md px-2 text-sm",
                        // activeItem === index
                        //   ? "text-secondary-foreground"
                        //   : "hover:text-accent-foreground",
                      )}
        ...
        {menuItems.map((item, index) => (
          <div key={index} className="relative">
            {item.href ? (
              ...
              >
                <Link href={item.href}>
                  <span className="text-text/90 hover:text-text flex items-center gap-2">
                    {item.icon}
                  </span>
                </Link>
              </Button>
